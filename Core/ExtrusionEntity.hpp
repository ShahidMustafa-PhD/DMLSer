...
public:
    ...
    float height;
	int LaserPower;  // watts
    /// Width of the extrusion.
    float SS; // Scan Speed  mm/sec.
    /// Height of the extrusion.
    float BeamDia; // laser Beam Diameter
...
//    ExtrusionPath(ExtrusionRole role, const Flow &flow) : role(role), mm3_per_mm(flow.mm3_per_mm()), width(flow.width), height(flow.height) {};
	...
	Lines translated(float x, float y);
...
typedef std::vector<ExtrusionPath*> ExtrusionPathPtrs;
