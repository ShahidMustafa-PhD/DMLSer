...
Polyline::lines() const
{
    ...
    if (this->points.size() >= 2) {
        ...
        for (Points::const_iterator it = this->points.begin(); it <= this->points.end()-1; ++it) {
          ...
          //for (int it = 0; it <= this->points.size()-1; ++i) {
             // if(it+1 < this->points.size())
			     //lines.push_back(Line(points.at(it), points.at(it+1)));
		}
    }
    return lines;
}
...
std::string
Polyline::wkt_SLM() const
{
    std::ostringstream wkt;
    wkt << " ";
    for (Points::const_iterator p = this->points.begin(); p != this->points.end(); ++p) {
        wkt << unscale(p->x) << " " <<  unscale(p->y);
        if (p != this->points.end()-1) wkt << " ";
    }
    wkt << " ";
    return wkt.str();
...
Polyline::scan_lines() const
{
    ...
    if (this->points.size() >= 2) 
        ...
        // if(it+1 < this->points.size())
        //lines.push_back(Line(points.at(it), points.at(it+1)));
	for (Points::const_iterator ptt = this->points.begin() ; ptt != this->points.end(); ++ptt) {
    
    //this method can cuase bug..
		 const auto index = std::distance(this->points.begin(), ptt);
		              if (index % 2) {
						 // Line L=Line(*(ptt-1),*ptt);
						 // L.translate(*(ptt-1),*ptt);
                lines.push_back(Line(*(ptt-1),*ptt));
		                                    }	
						}
