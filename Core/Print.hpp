...
class PrintObject
{
    ...
    /* (we use map instead of vector so that we don't have to worry about
       resizing it and the [] operator adds new items automatically) */
    std::map<size_t,std::vector<int> > region_volumes;
	std::map<size_t,  SurfaceCollection > support_structure_Polygons;
    std::map<size_t,Points> support_piller_positions;
	//(std::map<size_t,>::const_iterator);
    ...
    void _infill();
    void write_svg();
    void Support_pillers();
	   class SPillar : public Point {
        public:
        size_t top_layer, bottom_layer;
        SPillar(const Point &p) : Point(p), top_layer(0), bottom_layer(0) {};
    };
	std::vector<SPillar> s_pillars;
	std::map<size_t,std::vector<SPillar>> support_pillers;
...
class Print
{
	...
	std::string polygon_d(const ExPolygon &epolygon) const;
