...
void
Print::write_Polygons(const std::string &outputfile) const
{
  
	
    FILE* f = fopen(outputfile.c_str(), "w");
  
   const PrintObjectPtrs &obj = this->objects;
   PrintObject* ptrobj;
   Layer* ll;
   LayerRegion* lr;
 
  
            for (std::vector<PrintObject*>::const_iterator it = obj.begin(); it != obj.end(); ++it) {
	 ...
	 std::cout << "obj size: "<<obj.size() << "\n"<<std::endl;
			...
			{ ptrobj= obj[i];
			LayerPtrs Lp =ptrobj->layers;
std::cout << "LP size: "<<Lp.size() << "\n"<<std::endl;
			for (size_t j = 0; j < Lp.size(); ++j) 
			{ ll= Lp[j];
			  LayerRegionPtrs kk =ll->regions;
std::cout << "kk size: "<<kk.size() << "\n"<<std::endl; 
			for (size_t z = 0; z < kk.size(); ++z) 
			{ lr= kk[z];
			 SurfaceCollection fillsur =lr->fill_surfaces;
  Surfaces mysur= fillsur.surfaces;
  /*for (size_t n = 0; n < mysur.size(); ++n) 
			{  Surface mysur= mysur[n].surface;
			
			
			}*/
				std::cout << "mysur size: "<<mysur.size() << "\n"<<std::endl; 
			for ( Surfaces::const_iterator it =  mysur.begin(); it !=  mysur.end(); ++it) {
				ExPolygon ep=it->expolygon;
               std::string pd = this->polygon_d(ep);
                fprintf(f,"%s\n",pd.c_str());
				}
			}
			}
...
std::string
Print::polygon_d(const Polygon &polygon) const
{
...
std::string
Print::polygon_d(const ExPolygon &expolygon) const
{
    std::string pd;
...
std::string
Print::polygon_d(const Polyline &polyline) const
{
    ...
    for (Points::const_iterator p = polyline.points.begin(); p != polyline.points.end(); ++p) {
        d << unscale(p->x) << " ";
        d << unscale(p->y)  << " ";  // mirror Y coordinates as SVG uses downwards Y
    }
    //d << "Z";
    return d.str();
}
