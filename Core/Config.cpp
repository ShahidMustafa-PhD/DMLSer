...
ConfigBase::load(const std::string &file)
{
  ...
  BOOST_FOREACH(const pt::ptree::value_type &v, tree) { 
       ...
       // std::for_each(const pt::ptree::value_type &v, tree) {
...
DynamicConfig::read_cli(int argc, char** argv, t_config_option_keys* extra)
{
    ...
    for (const auto &oit : this->def->options) {  //  note:: options is a map.  already filled
	 //  during the  constructor of print_config _def and  Cli_config_def
	 //  and these are merged togather..
        std::string cli = oit.second.cli;  //  map<string,configoptionsdef>
