...
bool 
SurfaceCollection::contains(const Point &point) const
{   if (this->surfaces.empty()) return false;
for (Surfaces::const_iterator it= this->surfaces.begin(); it != this->surfaces.end(); ++it) {
...
bool
SurfaceCollection::RectGrids(float island_L,float island_H)

{  SurfaceCollection sur; 
if (this->surfaces.empty()) return false;
for (Surfaces::const_iterator it= this->surfaces.begin(); it != this->surfaces.end(); ++it) 
	   sur.append(it->RectGrids(island_L, island_H),it->surface_type);
    
   this->set(sur);
   return true;
}
bool
SurfaceCollection::contained_inGrown(const Point &point, const coord_t delta)
{  
...
for (Surfaces::const_iterator it= this->surfaces.begin(); it != this->surfaces.end(); ++it) 
	{    sur= offset((*it),delta);
  for (Surfaces::const_iterator is= sur.begin(); is != sur.end(); ++is) 
	   if (is->contains(point)) return true;
    }
    return false;
