...
{
    // Compute the translation to be applied to  meshes so that we work with smaller coordinates
    {
...
std::vector<coordf_t> PrintObject::generate_object_layers(coordf_t first_layer_height)
 {
   ...
   /* coordf_t min_layer_height = 0.0;
    ...
    coordf_t layer_height =this->config.layer_height.getFloat();//) std::min(min_nozzle_diameter, this->config.layer_height.getFloat());
    layer_height = this->adjust_layer_height(layer_height);
    this->config.layer_height.value = layer_height;
//std::cout << "Layer hieght\n" <<layer_height<<std::endl ;
    // respect first layer height
    //if(first_layer_height) {
      //  result.push_back(first_layer_height);
    //}
     //int number_of_layers=std::ceil (unscale(this->size.z)/layer_height);
  
	coordf_t print_z = first_layer_height;
    coordf_t height = 0;//first_layer_height;
      while (print_z < unscale(this->size.z)) {
              height = layer_height;
			   print_z += height;
//boost::nowide::cout << "hieght. " <<print_z  << std::endl;
...
void PrintObject::_slice()
{
	...
	boost::nowide::cout << "regions. " << this->print()->regions.size() << std::endl;
    coordf_t raft_height = this->config.Anchor_hieght.value;
    coordf_t first_layer_height ;//= this->config.first_layer_height.get_abs_value(this->config.layer_height.value);
    coordf_t Anchor_thickness= this->config.Anchor_layer_thickness.value;
   // this->model_object()->center_around_origin();
   //this->model_object()->arrange_volumes(); // arranges models on print bed.
   
    //this->model_object()->update_bounding_box();
   // -take raft layers into account
    int id = std::ceil(raft_height/Anchor_thickness);
    first_layer_height=raft_height;
   /* if (this->config.raft_layers > 0) {
	...
	
    {
        ...
        for (size_t i_layer = 0; i_layer < object_layers.size(); i_layer++) {
            ...
            coordf_t slice_z = hi - raft_height;//0.5 * (lo + hi) - raft_height;
    ...
    for (Layer* layer : this->layers) {
	  ...
	  boost::nowide::cout << "layers done: " << std::endl;
}
...
PrintObject::_slice_region(size_t region_id, std::vector<float> z, bool modifier)
{
    ...
    );  //  translates mesh  into quadrant 1.
...
void
PrintObject::write_svg()
{ //SVG *sv  = new SVG(std::string("Print")+std::string(".svg")); // allocate memory
FOREACH_LAYER(this, layer) 
...
//delete sv;
 ...
 void
PrintObject::Support_pillers()   // generate points following the shape of each island
{     
    std::cout << "Now Support Pillers: " << std::endl;
    Points pillars_pos;
	 
    coordf_t spacing ;//= scale_(this->config.support_material_spacing);
    //const coordf_t radius  = scale_(this->sm_pillars_radius());
    const coordf_t  ofset= scale_(0.5);
    ExPolygons  overhangs;
    //add anchors here// add  anchor layers here...
    size_t idd=this->layers.front()->id();
    coordf_t Anchor_thickness= this->config.Anchor_layer_thickness.value;
    Layer *prev = nullptr;
        coordf_t lo = 0;
        coordf_t hi = lo;
    //size_t id=idd;
    LayerPtrs anchors;
	   for (size_t i_layer = 1; i_layer <idd; ++i_layer) 
	   {
            lo = hi;  // store old value
            hi = Anchor_thickness*i_layer;
            coordf_t slice_z = hi;
			
           // Layer *layer =  new Layer this->add_layer(id++, hi - lo, hi, slice_z);
            Layer *lay =  new  Layer(i_layer,this, hi - lo, hi,slice_z);
			//slice_zs.push_back(float(slice_z));
            if (prev != nullptr) {
                prev->upper_layer = lay;
                lay->lower_layer = prev;
            }
            // Make sure all layers contain layer region objects for all regions.
            for (size_t region_id = 0; region_id < this->_print->regions.size(); ++ region_id)
                 lay->add_region(this->print()->regions[region_id]);
                 anchors.push_back(lay);//front()->id();
			     prev = lay;
        }
		
	this->layers.insert(this->layers.begin(),anchors.begin(),anchors.end());
 //-------------------------------------------------------------//

 
 for (std::map<size_t,SurfaceCollection>::const_iterator ke = this->support_structure_Polygons.begin(); ke != this->support_structure_Polygons.end(); ++ke)
 //for ( std::map<size_t,std::vector<int> >::const_iterator ke = this-> region_volumes.begin(); ke != region_volumes.end(); ++ke)
 { 
 
  size_t region_id=ke->first; 

	 
   spacing = scale_(this->layers[idd]->get_region(region_id)->region()->config.support_material_spacing);
   float oh_angle=0.01745*(this->layers[idd]->get_region(region_id)->region()->config.support_material_angle.value+1); //  radians   
   overhangs=ke->second;

  overhangs=union_ex(offset_ex(overhangs,spacing/3));
 // overhangs=offset_ex(overhangs,-spacing/2);
  BoundingBox pbb;
  //BoundingBoxf3  pbb=mesh.bounding_box() ;
  Points pp;
   
 for (ExPolygons::const_iterator it = overhangs.begin(); it != overhangs.end(); ++it) {
            
	  pbb=it->bounding_box();
      //std::cout << "bbox:" <<  pbb.min.x <<" " << pbb.min.y << " " <<  pbb.max.x <<" " << pbb.max.y <<"\n"<<std::endl ;	  
    for(coord_t ycc=pbb.min.y; ycc < pbb.max.y; ycc+=spacing)
        { // ycc+=ofset;//scale_(this->config.SLM_overlap.value);//HatchDistanInt;
        for(coord_t xcc=pbb.min.x; xcc < pbb.max.x; xcc+=spacing)
		{//xcc+=ofset;
		pp.push_back(Point(xcc,ycc));}
		}
 		
	}  
	if(!pp.empty())
	      {this->support_piller_positions[region_id]=pp;
	       pp.clear();}
 
 }	
	
	
	//  for all pillers identify  upper and lower layers....  
	for (std::map<size_t,Points>::const_iterator ke = this->support_piller_positions.begin(); ke != this->support_piller_positions.end(); ++ke)
	{  size_t region_id=ke->first;
     Points  pillars_pos= ke->second;
	 //float oh_angle=0.01745*(this->layers[idd+1]->get_region(region_id)->region()->config.support_material_angle.value+1); //  radians   
	// oh_permissible =scale_(this->layers[idd+1]->height/tan(oh_angle)); // mm	
		for (Points::const_iterator p = pillars_pos.begin(); p != pillars_pos.end(); ++p) {
            ...
            // check layers top-down
             ...
             for (int i = this->layers.size()-2; i >= 0; --i) {
			   ...
			   if (!this->layers[i]->regions[region_id]->fill_surfaces.contains(*p)) {
                    ...
                    if (pillar.top_layer > 0) {
						...
						//if(this->layers[i+1]->regions[region_id]->fill_surfaces.contained_inGrown(*p, -oh_permissible))
						{ pillar.top_layer = i;
					      object_hit = false;}//+ this->config.raft_layers;
                    }
                } else 
					{
                    if (pillar.top_layer > 0) {
                        // we have a pillar which is not needed anymore, so store it and initialize a new potential pillar
                        this->s_pillars.push_back(pillar);
                        pillar = SPillar(*p);
                    }
                    object_hit = true;
                }
            }
            if (pillar.top_layer > 0) this->s_pillars.push_back(pillar);
        }
	this->support_pillers[region_id]=this->s_pillars;
	this->s_pillars.clear();
	}

	//  now  populating layers and regions.

 
ExPolygons support_polygons;
Points gridp;
//coordf_t Rad=this->config.support_material_spacing / 1.0;
//populate the support  layers in every region...

//  move this to Layer  Region class.....
for (size_t i = 0; i < this->layers.size(); ++i) 
 {
  Layer &layer = *this->layers[i];
  std::cout << "Layer ID:" <<layer.id()<< "  hieght:" <<layer.slice_z<< "\n"<<std::endl ;
	for (std::map<size_t,std::vector<SPillar>>::const_iterator ke = this-> support_pillers.begin(); ke != this-> support_pillers.end(); ++ke) 
{     
size_t region_id=ke->first;
std::vector<SPillar> sm_pillars= ke->second;
coordf_t RB= layer.regions[region_id]->region()->config.tooth_base_length.value;
coordf_t RT= layer.regions[region_id]->region()->config.tooth_top_length.value;
coordf_t TH= layer.regions[region_id]->region()->config.tooth_hieght.value;
coordf_t taper_gain= ((RB-RT)/TH)*this->config.layer_height.value;
spacing =this->layers[idd]->get_region(region_id)->region()->config.support_material_spacing;
//coordf_t i_N=  TH/this->config.layer_height.value;
size_t  N_H=0;
size_t  D_N=0;
  if(1){
	 for (std::vector<SPillar>::const_iterator it = sm_pillars.begin(); it != sm_pillars.end(); ++it)
		 {
	    ...
	    // boost::nowide::cout << "before:" << Rad <<"\n"<< std::endl;
          ...
          float radius =fminf(spacing /1.2 ,(RT+taper_gain*D_N));
			//boost::nowide::cout << "after: " << radius<< "\n"<< std::endl;
		// radius=this->config.support_material_spacing/1.2;
			coord_t xcc=it->x;
			coord_t ycc=it->y;
	        ...
	        gridp.push_back(Point(xcc+Ra,ycc+Ra));
            gridp.push_back(Point(xcc+Ra,ycc-Ra));	
	        support_polygons.push_back(ExPolygon(Polygon(gridp)));
            }
	   ...
	   grown =diff_ex(support_polygons,offset_ex(ExPolygons(layer.regions[region_id]->fill_surfaces), scale_(layer.regions[region_id]->region()->config.tooth_no_support_offset.value)));
		
		//else grown=support_polygons;
			
			
 layer.regions[region_id]->support_surfaces.set(union_ex(grown),stSupport);// support.push_back(union_(grown));
 support_polygons.clear();
 grown.clear();
 } //  end if optinal
 
 //layer.regions[region_id]->support_surfaces.set(union_ex(diff_ex(overhangs,offset_ex(ExPolygons(layer.regions[region_id]->fill_surfaces),ofset))),stSupport);//union_ex(grown),stSupport);// support.push_back(union_(grown));
