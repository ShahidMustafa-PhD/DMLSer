...
Fill::new_from_type(const InfillPattern type)
{
   /*switch (type) {
        ...
        case ipSLMisland:           return new FillSLMisland();
		//case ipSLMipSLMstrips:      return new FillSLMstrips();
	    //case ipSLMchessboard:       return new FillSLMchessBoard();
        default: CONFESS("unknown type"); return NULL;
    }*/
	      return new FillSLMisland();// return new FillDMLSisland();
		     // return new DMLSFillGrid();
...
{    //  boost::nowide::cout << "inside Fill_surface1:\n "  << std::endl;
    //if (this->density == 0) return Polylines();

    // Perform offset.
  // Polygon outer=surface.expolygon.contour;
 ExPolygon expp=surface.expolygon;
  //if(outer.area() >= island_area_max)
      //expp = offset_ex(surface.expolygon, scale_(this->overlape_offset));
  // else
      //expp = offset_ex(surface.expolygon, -scale_(0.001)); // very little
   ...
   // for (size_t i = 0; i < expp.size(); ++i)
        this->_fill_surface_single( surface.thickness_layers,this->_infill_direction(surface),
            expp,&polylines_out);
