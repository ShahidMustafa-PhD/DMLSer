...
bool
SLMSlicer::print(int argc, char **argv)
{       
	...
	// -saves the name of config.ini files(--load  [name2].ini --load  [name2].ini)
   ...
   ModelVolume* stlvolumeptr;
	...
	for (const t_config_option_key &file : input_files)
    {
        ...
        try {
			...
			//stlvolumeptrs.push_back(stlvolumeptr);
			//slm_model = Model::read_from_file(file);
           } 
           catch (std::exception &e) 
           {
