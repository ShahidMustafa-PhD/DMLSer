        ...
        case 5:
            if (strcmp(name, "vertex") == 0) {
                ...
                if(!m_object)
                    this->stop();
				//strtol (pEnd,NULL,0);
                m_volume = add_volume(strtol(m_value[0].c_str(),NULL,0)*3, strtol(m_value[1].c_str(),NULL,0) * 3 + 2, static_cast<bool>(strtol(m_value[2].c_str(),NULL,0)));
                if(!m_volume)
                    this->stop();
...
TMFParserContext::get_transformations(std::string matrix, std::vector<double> &transformations)
{
    ...
    for (size_t i= 0; i < matrix.size(); i++)
        if ((matrix[i] == ' ' && !tmp.empty()) || (i == matrix.size() - 1 && !tmp.empty())) {
           //strtof (szOrbits, &pEnd);
		   m[k++] = std::strtof(tmp.c_str(),NULL);
            tmp = "";
        }else
            tmp += matrix[i];
    if(tmp != "")
        m[k++] = std::strtod(tmp.c_str(),NULL);
